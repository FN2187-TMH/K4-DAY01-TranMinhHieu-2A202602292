...
* **Record hạng 1:**
  ...
  * `class_name`: `"cab"`
  * `rank`: `1`
  * `score`: `0.510915` (tương đương $\approx 51.09\%$)
  * `taxonomy_name`: `"ImageNet-1K"`
* **Mô tả toàn ảnh:** Record này gán một nhãn phân loại tổng thể cho toàn bộ bức ảnh là chiếc xe taxi ("cab") với xác suất $51.09\%$, phản ánh chủ thể/ngữ cảnh chính của ảnh mà không xác định vị trí tọa độ cụ thể của vật thể.
* **Đơn vị định nghĩa class list:** Đơn vị/tác giả xây dựng bộ dữ liệu chuẩn **ImageNet-1K** (gồm 1,000 lớp đối tượng).
* **Lý do cần giữ ID, tên lớp và tên taxonomy:**
  * **ID (`class_id`)**: Đảm bảo tính nhất quán và tối ưu hóa khi xử lý bằng lập trình/máy tính.
  * **Tên lớp (`class_name`)**: Giúp con người (annotator, reviewer) đọc hiểu ngữ nghĩa trực quan.
  * **Tên taxonomy (`taxonomy_name`)**: Định danh không gian nhãn chuẩn (`ImageNet-1K`), tránh xung đột ngữ nghĩa khi chuyển đổi hoặc so sánh với các bộ dữ liệu khác (như COCO hay Pascal VOC).
...
* **Lý do model score không phải ground truth:** Model score chỉ là xác suất/độ tin cậy toán học (confidence score) được mô hình dự đoán dựa trên trọng số đã huấn luyện, có thể xuất hiện sai số hoặc nhiễu. Ground truth là thông tin nhãn chuẩn xác tuyệt đối được con người kiểm duyệt và gán thực tế.
...
* **Một record tiêu biểu (record 1 của sample `kitchen`):**
  ...
  * `bbox_width`: `113.58`
  * `bbox_height`: `279.68`
* **Diễn giải vị trí box bằng lời:** Đối tượng "person" được khoanh vùng bởi khung chữ nhật có tọa độ góc trên-bên trái là $(385.33, 69.24)$ px và góc dưới-bên phải là $(498.92, 348.92)$ px. Khung bao này có kích thước chiều rộng $113.58$ px và chiều cao $279.68$ px trên ảnh.
* **So sánh số prediction ở hai threshold:** Ở ngưỡng thấp (ví dụ $0.35$), số lượng bounding box trả về nhiều hơn, bao phủ được nhiều vật thể mờ/nhỏ nhưng lẫn nhiều nhiễu (dương tính giả). Ở ngưỡng cao (ví dụ $0.70$), số lượng box giảm đáng kể, chỉ giữ lại các đối tượng có độ tin cậy cao nhưng dễ bỏ sót vật thể bị che khuất (âm tính giả).
...
* **Một record tiêu biểu (instance `traffic-001`):**
  ...
  * `class_name`: `"bus"`
  * `score`: `0.925745`
  * Số điểm (`polygon_point_count`): `120`
  * Một phần `polygon_xy`: `[[148.0, 189.0], [147.0, 190.0], [145.0, 190.0], [143.0, 192.0], ...]`
...
* **Vai trò của `instance_id`:** `instance_id` dùng để phân biệt và định danh duy nhất từng đối tượng riêng lẻ cụ thể trong cùng một bức ảnh. Nó **không phải** là `class_id` (mã lớp chung) và **không phải** là `sample_id`/`coco_image_id` (mã nhận dạng bức ảnh).
...
| Tác vụ | Đơn vị/định dạng ground truth | Lỗi hoặc điểm mơ hồ quan sát được | Annotator làm gì? | Reviewer xem gì? |
| :--- | :--- | :--- | :--- | :--- |
| **Phân loại ảnh** | Nhãn ID/Text (Single/Multi-label Class ID, ví dụ: `468 - cab`). | Ảnh chứa nhiều chủ thể phức tạp dẫn đến mô hình chọn sai ngữ cảnh chính (ví dụ: ảnh bếp bị phân loại thành "gong"). | Đọc quy tắc ưu tiên trong guideline, chọn $1$ nhãn đại diện chính xác nhất cho ngữ cảnh toàn ảnh. | Kiểm tra nhãn gán có tuân thủ đúng thứ tự ưu tiên chủ thể trong guideline hay không. |
| **Phát hiện vật thể** | Tọa độ khung Bounding Box `[x_min, y_min, x_max, y_max]` kèm `class_id`. | Box bao quá rộng thừa nhiều nền, gán nhầm lớp các xe gần nhau, hoặc bỏ sót đối tượng nhỏ. | Vẽ khung bao ôm sát (tight) đường viền ngoại tiếp vật thể, gán đúng `class_id`. | Kiểm tra độ ôm sát của khung (IoU), phát hiện box thừa/thiếu và kiểm tra việc gán đúng nhãn. |
| **Instance segmentation** | Tập tọa độ đa giác `[(x1, y1), (x2, y2), ...]` kèm `instance_id` và `class_id`. | Viền mask chập chạp, lem sang vùng nền hoặc không phân tách rõ ranh giới giữa các instance sát nhau. | Chấm các điểm polygon bám chi tiết theo đường biên vật thể, gán đúng `instance_id` riêng. | Kiểm tra độ chính xác cấp độ pixel của đường biên, đảm bảo không trùng lấn mask giữa các instance. |
